...
4. **场景测试（2026-09-04 补齐，证据归档 `test-evidence/thinking-stream-20260904/`）**：
   需求逐字对照 6 场景全部实测并截图——S1 末尾摘要钉尾推进（连续帧摘要三变均贴尾）、S2 扫光带、
   S3 落定回退、S4 流式中点按展开全文、S5 展开再点收起、S6 重进会话历史行静态。
   未覆盖：真机观感、系统"减少动画"开关场景、极端超长行压测（见证据 README）。
5. 真机行为（触摸滚动观感、性能）未验证，需真机复核——明说不猜。
