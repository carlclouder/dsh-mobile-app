...
**状态：宿主机改「免令牌网关插件」方案，待重启生效（2026-09-16）——用户对宿主侧方案提出四条硬要求（逐字）：①"他应该是一个无感 伴随dsh启动跟随启动的东西"；②"而不是独立维护的"；③"dsh有很多 管理bat…完全无法配套管理"；④"不要动已有bat"。据此**否决**此前的外部代理进程 + 计划任务方案（会被安全软件拦截、关掉窗口即断连），改为 **dsh 插件 `dsh-auth-gateway`**：跑在 dsh 进程内的反向代理（监听 127.0.0.1:3081），用 `connection.authenticatedUrl()` 在进程内取令牌换会话 Cookie 并注入转发请求；dsh 启动即加载、dsh 退出即消失，**零独立进程、零计划任务、不碰任何 bat**。隔离实例实测：网关裸访问 200、对照直连 401、两端口同属 dsh 进程 PID；主 profile 已安装（bundles 在册）并完成重启前预验证（能正常启动，无 file-upload 冲突）。**待用户重启 dsh 生效**；生效后 Tailscale Serve 需指向 3081，并清理旧机制（计划任务 DSH-AuthProxy、`start-auth-proxy.vbs/.cmd`、外部 `tools/dsh-auth-proxy.mjs` 进程）。文档：`docs/dsh-auth-gateway-plugin.md`（设计/验证/部署/回滚）、`docs/DESIGN.md` §13（App 侧协议适配）、`docs/ARCHITECTURE.md`（访问链路）。**新坑（已固化）**：`dsh plugin add` 会把第三方 `dsh-file-upload` 重新登记进 bundles，与 0.1.5 内置同名会 `duplicate loader entry id` 启动即崩——每次 add 后必须复查并移除。**
