...
**状态：新版 dsh(0.1.5-rc.2) 全协议适配完成（2026-09-16，产出 release v0.1.175）——宿主升级后 App 从「不可达」到全功能可用，四层适配全部实测通过：①**令牌认证**：设置页粘贴带令牌链接（`https://.../?token=`）自动换会话 Cookie（30 天）并持久化，401 自愈；模拟器 debug 必须把 Host 改写为受信权威（否则 10.0.2.2 被围栏第一级判 403——曾误判"新版不检查 Host"而停用改写，已纠正）；②**RPC 双协议自适应**：新版把点号路径 `session.list` 改为斜杠 `session/list`、参数包一层 `args`（实测点号=404），实现"斜杠/点号双向自愈"（任一协议 404 即换另一种重试并记忆，防一次误判锁死）；③**轮询替代 WebSocket**：新版移除 /api/events.mux|host（带有效 Cookie 仍 404/断流，官方 WebUI 亦改 HTTP 轮询），EventStreamService 改为 3 秒周期 session/list 全量对账（状态机/通知/列表全保留，WS 代码保留不启动）；④**端点与参数对齐**：历史 `session.history`→`session/page`（必须带 throughSeq=会话游标、不能越界；游标两路解析：session/list 的 asOfSeq → 兜底从 "past cursor N" 错误解析；响应外层 `events`→`records`）、发消息 `session/prompt` 需 `requestId`（typert schema 实证）。模拟器实测（真实宿主 3080 + 有效令牌）：连接状态「已连接」、会话列表正常分组、进会话历史完整渲染、发消息后 Agent 真实回复。单测 101/101 绿（协议变更后同步更新了 DshApiClientTest 的路径/信封断言）、release 构建 0 错。证据 test-evidence/dsh-token-20260904/。未验证：真机 release（不含 debug Host 改写，走 https 域名天然受信）；审批/提问应答（respond 端点未在新版实测，无事件可触发）。工具：tools/dsh-auth-proxy.mjs（宿主侧自动登录代理）、tools/probe_*.py 与 extract_*.py（RPC 参数/端点探测与提取）。
