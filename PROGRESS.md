...
**状态：宿主机「免令牌网关」以认证链放行定稿，待重启生效（2026-09-16）——用户四条硬要求（逐字）：①“他应该是一个无感 伴随dsh启动跟随启动的东西”；②“而不是独立维护的”；③“dsh有很多 管理bat…完全无法配套管理”；④“不要动已有bat”。方案演进：外部代理进程+计划任务（否决：独立维护、可能被拦、误关窗口即断连）→ 改 bat 内联（否决：与 dsh 控制脚本无关联、每次调整都要动 bat）→ **dsh 插件 dsh-auth-gateway**（采纳）。插件跑在 dsh 进程内：①`connection.requestRejection` 放行 API/WS 升级；②`connection.authorizeIndex` 放行首页（实测只补 ① 时 API 200、根路径仍 401，v1.1.1 补齐）；判据 = 来源 loopback（Serve 是本机进程）+ Host 命中 `--trusted-host` 受信名单。**Tailscale Serve 保持 3080 不变，任何启动脚本都不用改**。安全含义：tailnet 内可达设备免令牌（公网仍不可达），关闭 `allowTailnetForwarded` 或卸载即恢复原生认证。隔离实例四项对照全部符合预期（Serve 风格 API/首页 200；本机裸 Host 401；Host 拼错 401）。主 profile 已装 v1.1.1（bundles 干净）；旧机制（计划任务 DSH-AuthProxy、start-auth-proxy.vbs/.cmd、外部代理进程、3081 占用）**已全部清理**。**已于 2026-09-16 22:29 随 dsh 重启生效并验证**：插件加载日志确认放行已开启（受信 Host 含当前域名）；真实域名 API/首页实测 200、本机裸 Host 对照 401；Tailscale Serve 仍指向 3080（用户脚本自动维护，未被改动）。**手机端尚未连通**：排查发现手机 Tailscale 离线（`tailscale status` 显示 `v2309a android offline, last seen 54m ago`），服务器侧全部通过，待手机恢复 Tailscale 连接后实测。文档：docs/dsh-auth-gateway-plugin.md（设计/方案演进/验证/运维/回滚）、docs/DESIGN.md §13、docs/ARCHITECTURE.md、USAGE.md（地址填法）。新坑已固化：plugin add 会把 dsh-file-upload 带回 bundles，每次装完必须复查移除。**

**状态：新版 dsh(0.1.5-rc.2) 全协议适配完成（2026-09-16，App 侧；release v0.1.178）——宿主升级后 App 从「不可达」到全功能可用，四层适配全部实测通过：①**令牌认证**：设置页粘贴带令牌链接（`https://.../?token=`）自动换会话 Cookie（30 天）并持久化，401 自愈；模拟器 debug 必须把 Host 改写为受信权威（否则 10.0.2.2 被围栏第一级判 403——曾误判"新版不检查 Host"而停用改写，已纠正）；②**RPC 双协议自适应**：新版把点号路径 `session.list` 改为斜杠 `session/list`、参数包一层 `args`（实测点号=404），实现"斜杠/点号双向自愈"（任一协议 404 即换另一种重试并记忆，防一次误判锁死）；③**轮询替代 WebSocket**：新版移除 /api/events.mux|host（带有效 Cookie 仍 404/断流，官方 WebUI 亦改 HTTP 轮询），EventStreamService 改为 3 秒周期 session/list 全量对账（状态机/通知/列表全保留，WS 代码保留不启动），会话页运行中每 2 秒轮询 history；④**端点与参数对齐**：历史 `session.history`→`session/page`（必须带 throughSeq=会话游标、不能越界；游标两路解析：session/list 的 asOfSeq → 兜底从 "past cursor N" 错误解析；响应外层 `events`→`records`）、发消息 `session/prompt` 需 `requestId`（typert schema 实证）；⑤**发送排队提示**：会话运行中发送会按 dsh 的 `mode=queue` 排队（官方 WebUI 同款行为），App 现提示"已加入队列…"避免误判未发送。另修：APK 内置默认地址由失效旧域名 tail98fa18 改为当前 taild10021（手机连不上的根因）。模拟器实测（真实宿主 + 有效令牌）：连接「已连接」、列表分组、历史完整渲染、发消息后 Agent 真实回复、裸地址与带令牌两种形式均可连。单测 101/101 绿、release 构建 0 错。证据 test-evidence/dsh-token-20260904/。未验证：真机 release（不含 debug Host 改写，走 https 域名天然受信）；审批/提问应答（respond 端点未在新版实测，无事件可触发）。工具：tools/dsh-auth-gateway/（网关插件源码）、tools/probe_*.py 与 extract_*.py（RPC 参数/端点探测与提取）。

**状态（历史）：新版 dsh 令牌认证适配——第一阶段完成（2026-09-16）——宿主 dsh 升级后启用一次性令牌认证（规则 21：令牌=入门钥匙一次性，Cookie=通行证 30 天），App 全部请求 401"不可达"。已实现：①新增 network/DshAuthSession.kt（地址拆分 base/?token=、令牌消费状态机 RAW→CONSUMED、Cookie SharedPreferences 持久化 30 天复用、authInterceptor 注入+401 自愈）；②DshApiClient 地址解析接入、httpClient/respondClient 挂 authInterceptor、openEventStream WS 握手注入 Cookie；③新增宿主工具 tools/dsh-auth-proxy.mjs（自动登录代理，实测 3083→3082 全链路通）。**
