"""生成 DSH 免令牌网关插件的一键安装/修复脚本（UTF-8 BOM，兼容 WinPS 5.1 与 pwsh 7）。

背景（2026-09-16 实测）：dsh 的 profile 清单只记录"实际装了什么"，`dsh plugin install`
不会补齐被删除的插件（输出 Already up to date），因此插件丢失后**必须重新 add**。
本脚本把"检查 → 安装 → 冲突复查 → 提示重启"固化成一条命令，供换机/重装/升级后恢复使用。
"""
path = r"D:\AI任务\dsh-mobile-app\tools\install-auth-gateway.ps1"
